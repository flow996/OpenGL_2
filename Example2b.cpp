...
void Win_SpecialKeyboard(int key, int x, int y) {
	...
	if( !m_bAutoRotation ) { // 沒有自動旋轉下才有作用
		...
		// 計算旋轉矩陣並更新到 Shader 中
		g_fYAngle += g_fDir * 0.025f;    // 旋轉角度遞增(遞減) 0.025
		// 計算旋轉矩陣並更新到 Shader 中
		ry = RotateY(g_fYAngle); //  degree 
		for( int i = 0 ; i < GRID_SIZE*GRID_SIZE ; i++ ) g_pGridQuad[i]->SetTRSMatrix(ry);
